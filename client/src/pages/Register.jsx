    ...
    <div className="min-h-screen bg-[var(--bg)] text-[var(--text-main)] theme-transition flex items-center justify-center p-6">
      <div className="w-full max-w-md">
        <div className="text-center mb-10">
          ...
          <h1 className="text-2xl font-bold">Join PollForge</h1>
          <p className="mt-2 text-[var(--text-muted)]">Start creating powerful polls today.</p>
        </div>

        <div className="rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-8 shadow-xl">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            <div>
              <label className="text-[10px] uppercase tracking-widest font-black text-[var(--text-muted)] mb-2 block">Full Name</label>
              <input
                {...register("name")}
                type="text"
              ...
              />
              {formState.errors.name && <p className="mt-1 text-xs text-rose-500">Enter your name.</p>}
            </div>
            <div>
          ...
          <div className="relative my-8">
            ...
            <div className="relative flex justify-center text-[10px] uppercase tracking-widest"><span className="bg-[var(--surface)] px-4 text-[var(--text-muted)]">Social Sign Up</span></div>
          </div>
        ...
        <p className="mt-8 text-center text-sm text-[var(--text-muted)]">
          Already have an account? <Link className="font-bold text-[var(--primary)] hover:underline" to="/login">Sign in</Link>
        </p>
      </div>
    </div>
  );
